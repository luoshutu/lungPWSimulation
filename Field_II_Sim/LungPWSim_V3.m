...
% 胸模线
sternumLine = round(hamming(10).*ceil(4e-3 / (z_size / N)));
sternumLine = interp(sternumLine,round(lineNumber/length(sternumLine)));
% 呼吸波形
breathRate  = 1;  %呼吸频率,Hz
breathNum   = ceil(prf / breathRate); %一次呼吸所占帧数
breathWave  = round(hamming(10).*ceil(4e-3 / (z_size / N)));
...
for frame_cnt = 1:imgFrame
    ...
    breathMove = ceil(breathWave(mod(frame_cnt, length(breathWave))+1));
    for i = 1:lineNumber                          % 获取二维仿体
        % 仿体线之间的相对移动加上随心跳的移动
        ster_line_position = round(sternumLine(mod(i+5*breathMove, length(sternumLine))+1));
        ster_line_mobile = frame_mobile - ster_line_position + breathMove;
        phantom = phantom_line(N, ster_line_mobile, i);
        PHANTOM(frame_cnt, :, i) = phantom;       % 存储仿体
    end
end
%% 仿体显示
% figure;
filename='D:\\_MyProject\\MATLAB\\lungPWSimulation\\Field_II_Sim\\phantom4.gif';          %输出路径+保存的文件名.gif
% while(1)
for frame_cnt = 1:imgFrame
    figure(frame_cnt);
...
timeGain = ones(1,N);
hamData  = 5e7*hamming(N/10);
timeGain(1:N/2) = 1e7;
timeGain(N/2:N) = 5e7;                    % 时间增益曲线
timeGain(N/2 - length(hamData)*(3/10): N/2) = hamData(length(hamData)*(2/10):length(hamData)/2);

for frame_cnt = 1:imgFrame
    singleFrame(:,:) = PHANTOM(frame_cnt,:,:);
    echoTemp = conv2(fp, singleFrame);
    echoBuff(frame_cnt,:,:) = echoTemp(1:N,1:lineNumber).*timeGain';
end
%% Echo显示
% figure;
filename='D:\\_MyProject\\MATLAB\\lungPWSimulation\\Field_II_Sim\\echo4.gif';          %输出路径+保存的文件名.gif
% while(1)
for frame_cnt = 1:imgFrame
%     figure(frame_cnt);
    imgEcho(:,:) = echoBuff(frame_cnt,:,:);
    imagesc(20*log(1 + abs(imgEcho)));
    pause(0.1);
    colormap(gray);
    title('卷积回波');
    set(gcf,'color','w');  %设置背景为白色
...
for frame_cnt = 1:imgFrame
%     figure;
    imgBModel(:,:) = Data_Amp(frame_cnt,1:D:end,:);
