...
clear Data_Amp;
%% 求arctan2,进行解调
sound_buff = zeros(15,5000-300+1);
for qq = 4:18
...
%         sig_Arctan2(xy) = atan2(sum(Data_I(xy,300:400)), sum(Data_Q(xy,300:400)));
        sig_Arctan2(xy) = atan2(sum(Data_I_fil(300:400,xy)), sum(Data_Q_fil(300:400,xy)));
    end
    % figure
    % plot(sig_Arctan2);
    sig_demod = unwrap(sig_Arctan2(1000:9000));               % 相位校正
...
%     legend('滤波前','滤波后');
    % axis([1 mm -100 100]);
%     sound(200*sig_demod_fil(300:5000),prf);
    sound_buff(qq-3,:) = 200*sig_demod_fil(300:5000);
end
